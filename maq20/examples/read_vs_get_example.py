...
module = maq20.get_module(
    1
)  # get module with a registration number of 1. maq20[1] returns the same thing.
...
result = module.read_data(
    start_channel=0, number_of_channels=module.read_input_channels()
)  # perform modbus request.
read_time = (
    time.time() - read_start_time
)  # record time taken by subtracting current time with initial time.

# Repeat for 'get' function.
get_start_time = time.time()
result_get = module.read_data(
    start_channel=0, number_of_channels=module.get_number_of_channels()
)
